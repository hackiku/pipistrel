...
b = Variable("Wingspan", 9.147, "b", "m")
S = Variable("Wing Area", 9.147, "S", "m²")
aspect_ratio = Variable("Aspect Ratio", 3.888, r"\lambda")
n = Variable("Taper Ratio", 0.520, "n")
...
# glauert inputs
c_z_krst = Variable("Cruise Lift Coefficient", 0.316, r"C_{z_{krst}}", "")
specop = Variable("Specific Load", 800, r"\sigma_{op}", "N/m^2")

c_z_maxap_s = Variable("Max Lift Coefficient at Root", 1.62, r"C_{z_{maxap\_s}}", "")
c_z_maxap_0 = Variable("Max Lift Coefficient at Tip", 1.78, r"C_{z_{maxap\_0}}", "")
aaap_s = Variable("Lift Gradient at Root", 0.113, r"AAAP\_S", "1/°")
aaap_0 = Variable("Lift Gradient at Tip", 0.100, r"AAAP\_0", "1/°")
anap_s = Variable("Zero Lift Angle at Root", -2.8, r"\alpha_{nap\_s}", "°")
anap_0 = Variable("Zero Lift Angle at Tip", -3.0, r"\alpha_{nap\_0}", "°")


# results from glauert
...
def main():
    ...
    with st.expander("Calculate wing aspect ratio (λ)"):
        ...
        def calculate_aspect_ratio():
            aspect_ratio.value = b.value**2 / S.value
            numbers = "=" + f"\\frac{{{b.value:.2f}^2}}{{{S.value:.2f}}}"
            aspect_ratio.formula = f"\\lambda = \\frac{{{b.latex}^2}}{{{S.latex}}} {numbers}"
        ...
        with col4:
          st.latex(f"{S.latex} = {S.value:.3f} \ {S.unit}")
            # st.latex = (S.latex)
        # st.latex(aspect_ratio.formula)               
    
    calculate_aspect_ratio()
    variables_two_columns(aspect_ratio, display_formula=True) # λ
    ...
    C            preseka                     [step.]  [km/h]   [kg/m^3]
        ...
        &     /    16,      {aspect_ratio.value:.3f},   {n.value:.3f},      0.0,    {v_krst.value},    {rho.value} /

        DATA CZMAXAP_S / {c_z_maxap_s.value} / ! maks. koef. uzgona ap. u korenu krila
        DATA CZMAXAP_0 / {c_z_maxap_0.value} / ! maks. koef. uzgona ap. na kraju krila
        DATA AAAP_S / {aaap_s.value} / !grad. uzgona ap. u korenu [1/o]
        DATA AAAP_0 / {aaap_0.value} / !grad. uzgona ap. na kraju [1/o]
        !teorijska  vrednost gradijenta uzgona 2PI = 0.1096622 [1/o]
        DATA ANAP_S / {anap_s.value} / !ugao nultog uzgona ap. u korenu krila [o]
        DATA ANAP_0 / {anap_0.value} / !ugao nultog uzgona ap. na kraju krila [o]
        DATA LS / {l_s.value} /  ! duzina tetive u korenu krila u metrima
